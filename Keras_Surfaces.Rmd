...
for(s_id in x_series$series_id) {
	...
	x_dwt <- modwt(z$PC1, wf="haar", n.levels=7, boundary="periodic")
	tmp_01 <- bind_rows(
		tmp_01, 
		bind_cols(data.frame(cols = columns_01), data.frame(PC1 = abs(fft(x_dwt$d2)) )) %>% spread(cols, PC1)
		)
	
	x_dwt <- modwt(z$PC2, wf="haar", n.levels=7, boundary="periodic")
	tmp_02 <- bind_rows(
		tmp_02, 
		bind_cols(data.frame(cols = columns_02), data.frame(PC2 = abs(fft(x_dwt$d2)) )) %>% spread(cols, PC2)
		)
	
	x_dwt <- modwt(z$PC3, wf="haar", n.levels=7, boundary="periodic")
	tmp_03 <- bind_rows(
		tmp_03, 
		bind_cols(data.frame(cols = columns_03), data.frame(PC3 =  abs(fft(x_dwt$d2)) )) %>% spread(cols, PC3)
		)	
}
