...
p <- ggplot(y) + geom_line(aes(x, abs(fft(PC1, inverse = TRUE)), col=1))  #+ geom_line(aes(x, Re(fft(PC2)), col=2)) #+ geom_line(aes(x, PC3, col=3))
p + facet_grid(rows = vars(series_id, group_id))
```
...
library(waveslim)
z <- x_train_pca %>% filter(series_id == 3) 
dwt(z$PC1, wf="la8", n.levels=4, boundary="periodic")
# dwt.nondyadic(z$PC1)

z <- x_train_pca %>% filter(series_id == 200) 
dwt(z$PC1, wf="la8", n.levels=4, boundary="periodic")
