...
```{r}
set.seed(100)
myfolds <- createMultiFolds(x_train_for_train$surface, k = 5, times = 10)
control <- trainControl("repeatedcv", index = myfolds, selectionFunction = "oneSE")
 
# Train PLS model
mod1 <- train(surface ~ ., data = x_train_for_train,
 method = "pls",
 metric = "Accuracy",
 tuneLength = 20,
 trControl = control,
...
```{r}
mod2 <- train(surface ~ ., data = as.matrix(x_train_for_train),
method = "lda",
metric = "Accuracy",
trControl = control,
...
y_hat <- predict(fit_model_rf_wood, x_train_for_test)
y_test <- x_train_for_test$surface
...
```{r warning=FALSE}
fit <- train(surface ~ . , method = "lda", data = x_train_for_train)
y_hat <- predict(fit, x_train_for_test, type = "raw")
y_test <- x_train_for_test$surface
...
fit_knn3 <- knn3(surface ~ ., data = x_train_for_train, k = 33)
y_hat <- predict(fit_knn3, x_train_for_test, type = "class")
y_test <- x_train_for_test$surface
